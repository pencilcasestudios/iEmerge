...
namespace :deploy do
  ...
  task :sync_assets do
    ...
    run_locally "rsync --verbose --stats --progress --compress --archive --partial --recursive --times --perms --links --delete --exclude '.*' --delete-excluded ~/Projects/Rails/Galaunia/app/assets/images/ #{fetch(:user)}@#{DEPLOYMENT_CONFIG["production_server_name"]}:#{fetch(:asset_vault_path)}/#{fetch(:application)}/Images/"
    run_locally "rsync --verbose --stats --progress --compress --archive --partial --recursive --times --perms --links --delete --exclude '.*' --delete-excluded ~/Projects/Rails/Galaunia/app/assets/pdfs/ #{fetch(:user)}@#{DEPLOYMENT_CONFIG["production_server_name"]}:#{fetch(:asset_vault_path)}/#{fetch(:application)}/PDFs/"
