...
namespace :deploy do
  ...
  task :sync_assets do
    ...
    run_locally "rsync --verbose --stats --progress --compress --archive --partial --recursive --times --perms --links --delete --exclude '.*' --delete-excluded ~/Projects/Rails/Galaunia/app/assets/images/ #{fetch(:deploy_user)}@#{DEPLOYMENT_CONFIG["staging_server_name"]}:#{fetch(:asset_vault_path)}/#{fetch(:application)}/Images/"
    run_locally "rsync --verbose --stats --progress --compress --archive --partial --recursive --times --perms --links --delete --exclude '.*' --delete-excluded ~/Projects/Rails/Galaunia/app/assets/pdfs/ #{fetch(:deploy_user)}@#{DEPLOYMENT_CONFIG["staging_server_name"]}:#{fetch(:asset_vault_path)}/#{fetch(:application)}/PDFs/"
